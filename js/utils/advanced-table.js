	...
	$.fn.advancedtable = function(custom) {
		...
		function redrawTable() {
			...
			function redrawNavigation(){
				...
				if(true){
					blockNavigation += '<ul>';
					if(pages>settings.maxDisplay){
						blockNavigation += '<li class="page-prev" ><a href="javascript:void();"><i class="icon icon-chevron-left"></i></a></li>';
					}
					...
					if(pages>settings.maxDisplay){
						blockNavigation += '<li class="page-next" ><a href="javascript:void();"><i class="icon icon-chevron-right"></i></a></li>';
					}
					blockNavigation += '</ul>';
				}else{
		...
		function setActivePage(number,navi){
			...
			if(settings.resultElement != ""){
				$(settings.resultElement).find('.tableInfo').html("Showing " + (from + 1) + " - " + ((tableRows < to) ? tableRows : to) + " of " + tableRows);
			}else{
